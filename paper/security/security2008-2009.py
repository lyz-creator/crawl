...
# 目标网页 URL（2008年）
url = 'https://www.usenix.org/legacy/events/sec08/tech/'

# 下载目录
download_folder = r"C:\Users\lyz\Desktop\crawler\Security\Security_2008"
...
if response.status_code == 200:
    ...
    # 找到所有 class 为 'techdesc' 的 p 标签
    paper_tags = soup.find_all('p', class_='techdesc')
    ...
    for paper_tag in paper_tags:
        ...
        if title_tag:
            ...
            # 找到下一个 class 为 'paperfiles' 的 p 标签
            next_p = paper_tag.find_next_sibling('p', class_='paperfiles')
            if next_p:
                # 在 'paperfiles' 标签内找到包含PDF链接的 <a> 标签
                pdf_link_tag = next_p.find('a', string='PDF')
                if pdf_link_tag and pdf_link_tag.get('href'):
                    pdf_link = 'https://www.usenix.org/legacy/events/sec08/tech/' + pdf_link_tag.get('href')
